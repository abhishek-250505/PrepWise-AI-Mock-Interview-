...
function Step2Interview({ interviewData, onFinish }) {
  ...
  const submitAnswer = async () => {
    if (isSubmitting || hasLeftRef.current) return;
    stopMic()
...
setIsSubmitting(false)
  ...
  const handleLeaveInterview = async () => {
    ...
    const shouldLeave = await confirmToast("Leave this mock interview? Your test will stop now.", {
      confirmText: "Leave Mock",
      cancelText: "Stay",
    });
    ...
    try {
      await axios.post(ServerUrl + "/api/interview/cancel", { interviewId }, { withCredentials: true });
    } catch (error) {
      console.log(error);
    } finally {
      navigate("/");
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
         ...
         {!feedback ? ( <div className='flex items-center gap-4 mt-6'>
            ...
            className='mt-6 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl shadow-sm'>
              ...
              {idealAnswer && (
                <div className='mb-4 rounded-xl border border-emerald-100 bg-white p-4'>
                  <p className='mb-2 text-sm font-semibold text-gray-800'>Strong sample answer</p>
                  <p className='text-sm leading-relaxed text-gray-600'>{idealAnswer}</p>
                </div>
              )}
